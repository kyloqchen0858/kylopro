...
if %errorlevel% == 0 (
    echo [OK] 任务注册成功！
    echo.
    echo 任务名称: Kylopro-Nexus-Gateway
    echo 触发条件: 用户登录后 15 秒自动启动
    echo.
    echo 管理命令:
    echo   查看状态: schtasks /query /tn Kylopro-Nexus-Gateway
    echo   立即运行: schtasks /run /tn Kylopro-Nexus-Gateway
    echo   停止任务: schtasks /end /tn Kylopro-Nexus-Gateway
    echo   删除任务: schtasks /delete /tn Kylopro-Nexus-Gateway /f
) else (
