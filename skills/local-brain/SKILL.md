...
## 财务与配额管理（原 cost-manager，已整合）

Kylopro 运行产生实际费用（模型 token + API 调用）。**原则：能用程序/免费工具完成的，不消耗 API 额度。**
...
| 日常轻量搜索 | `ddg_search` | 避免月底配额耗尽 |

### Tavily 配额守则
- 月免费：**1000 次**（每月 1 日重置）
- 剩余 < 100 次：仅关键任务
- 剩余 < 20 次：全部切 DuckDuckGo
...
|------|---------|------|
| 🟢 正常 | 余额 > 20% | 正常运行 |
| 🟡 预警 | 余额 10%~20% | 优先本地执行，减少模型调用 |
| 🔴 停止 | 余额 < 5% | 停止非关键 API 调用，仅用免费工具 |

### 节约策略（按优先级）
1. 本地执行优先：能用 `run_terminal` / `local_think(run_code)` 完成的，不调云模型
2. 文件工具优先：读/写/搜索代码用 `read_file`/`write_file`/`exec grep`
3. 批量合并：多个小问题合并为一次模型调用
4. L1 优先：不随意升级到 deepseek-reasoner
5. ddg_search 替代 tavily_search
